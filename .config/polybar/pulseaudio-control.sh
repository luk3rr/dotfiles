...
FORMAT='%{F#555}$VOL_ICON%{F-} ${VOL_LEVEL}% $NODE_NICKNAME'
...
function output() {
    ...
    if [ "$NODE_TYPE" == "output" ]; then
        ...
        if [ "$iconsLen" -ne 0 ]; then
            if [ "$NODE_NICKNAME" == "Headphone" ]; then
...
while [[ "$1" = --* ]]; do
    ...
    case "$arg" in
        ...
        --icons-muted)
            if getOptVal "$@"; then shift; fi
            IFS=, read -r -a ICONS_MUTED <<< "${val//[[:space:]]/}"
            ;;
        --icon-node)
            if getOptVal "$@"; then shift; fi
            # shellcheck disable=SC2034
